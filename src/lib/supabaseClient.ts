// Cliente do Supabase pra comunicar com o banco de dados
import { createClient } from '@supabase/supabase-js';
import { projectId, publicAnonKey } from '../utils/supabase/info';

// Monta a URL do Supabase a partir do ID do projeto
const supabaseUrl = `https://${projectId}.supabase.co`;
const supabase = createClient(supabaseUrl, publicAnonKey);

// Log pra confirmar que conectou
...
// Perfil de um jogador/integrante da família
export interface Perfil {
  id: string;
  nome_integrante: string;
  avatar: string;                 // Emoji escolhido
  token_familiar: string;          // Código de 6 caracteres da família
  pontos: number;                  // Pontuação total
  precisao: number;                // % de acertos (0-100)
  tempo_resposta_medio: number;    // Em segundos
  total_jogadas: number;           // Quantidade de jogadas
  desempenho_por_dificuldade: {    // Estatísticas separadas por nível
...
// Registro de uma jogada individual
export interface HistoricoJogada {
  id: string;
  jogador_id: string;
  jogo: string;                    // Ex: 'quiz', 'sorting', etc
  nivel: number;                   // 1 a 10
  acerto: boolean;                 // Acertou ou errou?
  tempo_resposta: number;          // Quanto tempo levou
  pontuacao: number;               // Pontos ganhos
  dificuldade: string;             // 'Fácil', 'Médio' ou 'Difícil'
  dados_adicionais?: Record<string, any>;  // Info extra específica do jogo
  timestamp: string;               // Quando aconteceu
}

// Entrada na tabela de ranking familiar
...
// Ranking formatado pra mostrar na tela
export interface RankingFamiliaDetalhado {
  posicao: number;               // 1º, 2º, 3º...
  nome_integrante: string;
  avatar: string;
  pontos: number;
  precisao: number;
...
// Estatísticas gerais do sistema todo
...
class SupabaseClient {
  ...
  // Testa se consegue conectar no banco e se as tabelas existem
