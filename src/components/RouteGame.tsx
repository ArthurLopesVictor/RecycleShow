...
export function RouteGame({ onBack, difficulty }: RouteGameProps) {
  ...
  const { addMove, clearMoves } = usePendingMoves(gameOver, currentPlayer.id, 'route', difficulty);
  ...
  const generateCollectionPoints = (): CollectionPoint[] => {
    ...
    for (let i = 0; i < pointCount; i++) {
      newPoints.push({
        ...
        x: Math.random() * 80 + 10,
        y: Math.random() * 70 + 15,
        volume: Math.floor(Math.random() * 8) + 2,
        collected: false
      });
    }
  ...
  useEffect(() => {
    setPoints(generateCollectionPoints());
  }, [difficulty]);
