...
// Componente principal que gerencia a seleção e exibição dos minigames
// É o "hub" central onde o jogador escolhe qual jogo quer jogar
export function Minigames() {
  const { currentPlayer } = usePlayerSupabase();
  
  // Guarda qual jogo está sendo jogado atualmente e em qual nível
  const [selectedGame, setSelectedGame] = useState<{
    type: 'quiz' | 'sorting' | 'route' | 'memory' | 'composting' | null;
    difficulty: number;
  }>({ type: null, difficulty: 1 });

  // Controla quais categorias de jogos estão expandidas (mostrando todos os níveis)
  ...
  // Armazena a porcentagem de progresso de cada nível de cada jogo
  // Vem direto do Supabase quando o componente carrega
  ...
  // Guarda quais níveis estão bloqueados (ainda não podem ser jogados)
  ...
  // Carrega todo o progresso do jogador quando o componente aparece
  // ou quando muda de jogador
  useEffect(() => {
    const loadProgress = async () => {
      try {
        setLoading(true);
        // Busca os dados de progresso do banco
        const progressData = await supabaseClient.obterProgressoCompleto(currentPlayer.id);
        setProgress(progressData);

        // Agora verifica quais níveis estão bloqueados
        ...
        // Pra cada jogo e cada nível, verifica se está desbloqueado
  ...
  // Pega o progresso de cada jogo específico do estado carregado
  ...
  // Função pra expandir/colapsar uma categoria de jogos
  ...
  // Quando o jogador termina um jogo, volta pro menu e atualiza o progresso
  const handleGameComplete = async () => {
    setSelectedGame({ type: null, difficulty: 1 });
    // Recarregar progresso pra refletir as mudanças
    try {
      const progressData = await supabaseClient.obterProgressoCompleto(currentPlayer.id);
      setProgress(progressData);

      // Recalcular níveis bloqueados (talvez desbloqueou algo novo!)
      const newLockedLevels: any = {
        quiz: new Set(),
        sorting: new Set(),
        route: new Set(),
  ...
  // Se tem um jogo selecionado, mostra a tela daquele jogo específico
  ...
  // Mostra um spinner enquanto carrega o progresso do banco
  if (loading) {
    return (
  ...
  // Menu principal mostrando todos os jogos disponíveis
  return (
    <div className="space-y-6 md:space-y-8">
      {/* Seção do Quiz de Conhecimento */}
      <div className="bg-white/50 backdrop-blur-sm rounded-2xl p-4 md:p-6 border-2 border-green-200 shadow-lg">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 gap-3">
          ...
          {/* Botão pra mostrar mais ou menos níveis */}
          <Button
        ...
        {/* Grid com os cards dos níveis (3 ou todos os 10) */}
      ...
      {/* Seção da Separação de Resíduos - estrutura similar ao Quiz */}
      <div className="bg-white/50 backdrop-blur-sm rounded-2xl p-4 md:p-6 border-2 border-green-200 shadow-lg">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 gap-3">
          <div className="flex items-center gap-3">
      ...
      {/* Banner informativo explicando o sistema de progressão */}
      <div className="bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-2xl p-6 shadow-xl">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            {/* Legenda de cores por dificuldade */}
