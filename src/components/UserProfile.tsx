...
// Tela de perfil do jogador mostrando todas as estatísticas e conquistas
// Exibe pontos, precisão, tempo médio, desempenho por dificuldade e medalhas
export function UserProfile() {
  const { currentPlayer } = usePlayerSupabase();
  const user = currentPlayer;
  // Guarda todas as jogadas do usuário pra calcular as estatísticas
  const [metrics, setMetrics] = useState<HistoricoJogada[]>([]);
  const [loading, setLoading] = useState(true);

  // Carrega o histórico completo de jogadas do banco quando abre o perfil
  useEffect(() => {
    const loadMetrics = async () => {
      try {
        setLoading(true);
        // Pega até 1000 jogadas do histórico
  ...
  // Calcula as estatísticas principais do jogador
  ...
  // Separa o desempenho por dificuldade (fácil, médio, difícil)
  // Baseado nos níveis: 1-3 = fácil, 4-7 = médio, 8-10 = difícil
  ...
  metrics.forEach(m => {
    // Descobre a categoria da jogada
    let category: 'easy' | 'medium' | 'hard' = 'easy';
    
    if (m.dificuldade) {
      // Se tiver a coluna dificuldade, usa ela
  ...
  // Função pra exportar os dados em formato JSON
  ...
  // Função pra exportar os dados em formato CSV (planilha)
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      {/* Cabeçalho do perfil com avatar e informações principais */}
      <Card className="bg-gradient-to-br from-green-500 to-emerald-600 text-white shadow-xl border-0 overflow-hidden">
        <CardContent className="pt-4 md:pt-6">
          <div className="flex flex-col sm:flex-row items-center gap-4 md:gap-6">
            {/* Avatar grande e bonito do jogador */}
      ...
      {/* Grid com as 4 estatísticas principais */}
      ...
      {/* Análise de desempenho por dificuldade com barras de progresso */}
      <Card className="shadow-md border-2 border-green-100">
        <CardHeader>
      ...
      {/* Seção de conquistas/medalhas desbloqueáveis */}
      <Card className="shadow-md border-2 border-green-100">
        <CardHeader>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
            {/* Medalha: Primeiros Passos (10 pontos) */}
            ...
            {/* Medalha: Especialista (100 pontos) */}
            <div className={`p-3 md:p-4 rounded-lg border-2 text-center transition-all ${
            ...
            {/* Medalha: Precisão (90% de acertos) */}
            <div className={`p-3 md:p-4 rounded-lg border-2 text-center transition-all ${
      ...
      {/* Área de exportação de dados para análise externa */}
      <Card className="shadow-md border-2 border-green-100">
        <CardHeader>
        ...
        <CardContent>
          ...
          {/* Mensagem caso não tenha dados ainda */}
          {metrics.length === 0 && (
          ...
          {/* Info sobre quantas interações foram registradas */}
          {metrics.length > 0 && (
