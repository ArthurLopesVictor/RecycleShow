...
// Propriedades que cada card de jogo recebe
...
// Card visual que representa cada minigame na tela principal
// Mostra informações do jogo, nível atual, progresso e botão para jogar
export function GameCard({
  ...
  return (
    <motion.div
      // Anima o card quando o mouse passa por cima (só se não estiver bloqueado)
      ...
      <Card className={`transition-all shadow-md hover:shadow-xl border-2 ${
        ...
        <CardHeader className="pb-3">
          <div className="flex items-start justify-between mb-3">
            {/* Ícone do jogo com efeito de brilho ao fundo */}
            <div className="relative">
              {/* Camada de blur colorida pra dar aquele efeito de glow */}
              <div 
                className="absolute inset-0 rounded-xl blur-md opacity-30"
                style={{ backgroundColor: color }}
              ></div>
              {/* Box colorido com o ícone do jogo */}
              <div 
        ...
        <CardContent className="pt-0">
          <div className="space-y-4">
            {/* Barra de progresso - só aparece se já tiver jogado e não estiver bloqueado */}
            {!locked && progress > 0 && (
              <div>
            ...
            {/* Mensagem explicando porque o jogo está bloqueado */}
            {locked && (
